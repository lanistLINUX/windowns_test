...
<div class="container-fluid">
	...
	<ul role="tablist" class="nav nav-tabs">
		<li <c:if test="${type==0 }">class="active"</c:if> role="presentation"><a href="../admin/goods_list.jsp">全部商品</a></li>
		<li <c:if test="${type==1 }">class="active"</c:if> role="presentation"><a href="../admin/goods_list.jsp?type=1">条幅推荐</a></li>
		<li <c:if test="${type==2 }">class="active"</c:if> role="presentation"><a href="../admin/goods_list.jsp?type=2">热销推荐</a></li>
		<li <c:if test="${type==3 }">class="active"</c:if> role="presentation"><a href="../admin/goods_list.jsp?type=3">新品推荐</a></li>
	</ul>
	...
	<table class="table table-bordered table-hover">
		...
		<c:forEach items="${allAccessories }" var="g">
			<tr>
				<td><p>${g.a_id }</p></td>
				<td><p><a href="/goods_detail?id=${g.a_id}" target="_blank"><img src="${g.a_url}" width="100px" height="100px"></a></p></td>
				<td><p><a href="/goods_detail?id=${g.a_id}" target="_blank">${g.a_name}</a></p></td>
				<td><p>${g.a_text}</p></td>
				<td><p>${g.a_price}</p></td>
				<td><p>${g.a_num}</p></td>
				<td>
					<p>
					 ...
					 <a class="btn btn-success" href="/admin/goods_editshow?id=${g.a_id }& pageNumber=${p.a_num}&type=${p.a_style}">修改</a>
					<a class="btn btn-danger" href="/admin/goods_delete?id=${g.a_id }&pageNumber=${p.a_num}&type=${p.a_style}">删除</a> 
				</td>
			</tr>
		</c:forEach>
