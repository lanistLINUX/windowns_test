    ...
    <div class="container">
        ...
        <div class="split row">
            <div class="col-xs-12 col-sm-3" style="text-align: center;">
                <select name="kind" id="select" onchange="mabr(this)">
...
<!--         <div class="split_info">
                ...
                <div class="col-xs-12 col-sm-12">
                    <div class="row">
                        ...
                        <div class="info_img_big col-xs-12 col-md-40ths col-sm-6 col-lg-40ths only_desc" id="big"></div>
	...
	<c:if test="${pageNum!=1}">
		<li>
				<a  href="../get_HanFuPage?pageNum=1" >首页</a>
		<%-- <a href="limitServlet?pageNum=${pageNum-1 }" >上一页</a> --%>
		</li> 
		 <li>
		 <!--  -->
	 		  <a href="../get_HanFuPage?pageNum=${pageNum-1 }" aria-label="Previous">
 	 		   <span aria-hidden="true">&laquo;</span>
  	       </a>
 	     </li>
		</c:if>
                                                          
                                <li class="active"><a id="page1">${pageNum }</a></li>
      
<%--                                  <c:if test="${pageCount!=pageNum}"> --%>
                                <li>
					<%-- <span >当前在第${pageNum }页</span> --%>
                                <!-- 下一页箭头 -->
                                    <a  aria-label="Next" onclick="pageNext(${pageNum+1 })">
                                        <span aria-hidden="true" >&raquo;</span>
                                    </a>
                                    </li>
				<%-- <a href="limitServlet?pageNum=${pageNum+1 }" >下一页</a> --%>
					 <li>
						<a href="../get_HanFuPaget?pageNum=${pageCount }" >末页</a>
               ...
               <li id="pageCount">总共有${pageCount }页</li>
               </nav>
...
function mabr(obj){
	   ...
	   $.ajax({
			...
			url:"../get_HanFu",
			type:"post",
			data:{name:name,pageNum:1},
			success:function(data){
			var hanfu = JSON.parse(data);
			
			for(var i=0;i<hanfu.length;i++){
				
				$("#small").append(" <div class=\"info_img_small col-xs-12 col-sm-6 col-md-4\"><a href=\"Hanfu_Detail.jsp\" target=\"_blank\"><img height=\"200px\" class=\"img-responsive\" src=\""+hanfu[i].url+"\" alt=\"分类图片\"></a></div>");
			} 
...
$.ajax({
	...
	url:"../get_HanFu",
	type:"post",
	data:{name:name,pageNum:pageCount},
	success:function(data){
	var hanfu = JSON.parse(data);
...
function ajaxPage(name,num){
	...
	$.ajax({
			...
			url:"../get_HanFuPage",
			type:"post",
			data:{name:name,pageNum:num},
			success:function(data){
				var data =JSON.parse(data);
				
			$("#pageCount").html("总共有"+data.pageCount+"页");
			$("#page1").html(1);
